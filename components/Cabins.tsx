      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 md:mb-16">
          <div className="inline-flex items-center gap-2 bg-white border border-[hsl(195,85%,82%)] rounded-full px-4 py-1.5 mb-4">
            ...
            <span className="text-[hsl(197,80%,30%)] text-sm font-semibold">আমাদের Cabins</span>
          </div>
          ...
          <p className="text-slate-500 text-sm sm:text-base md:text-lg max-w-2xl mx-auto">
            প্রতিটি কেবিন আপনার আরাম ও গোপনীয়তার কথা মাথায় রেখে ডিজাইন করা হয়েছে।
          </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mb-8 md:mb-12">
          <div className="flex items-center gap-3 bg-white rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 shadow-sm border border-slate-100">
            ...
            <div>
              <div className="font-bold text-slate-800 text-sm">কেবিন বুকিং</div>
              <div className="text-slate-500 text-xs">একটি বা একাধিক কেবিন বুক করুন</div>
            </div>
          </div>
          <div className="flex items-center gap-3 bg-white rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 shadow-sm border border-slate-100">
            ...
            <div>
              <div className="font-bold text-slate-800 text-sm">Full Boat বুকিং</div>
              <div className="text-slate-500 text-xs">পুরো বোট এক্সক্লুসিভলি বুক করুন</div>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="relative h-44 sm:h-52 overflow-hidden">
                ...
                <div className="absolute top-2.5 left-2.5 flex gap-1.5 flex-wrap max-w-[70%]">
                  {cabin.badge && (
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${badgeStyles[cabin.badge] || 'bg-slate-700 text-white'}`}>
                      {cabin.badge}
                    </span>
                  )}
                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${cabin.available ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}`}>
                ...
                <div className="absolute bottom-2.5 right-2.5 bg-white/95 backdrop-blur-sm rounded-lg sm:rounded-xl px-2.5 py-1">
                  <span className="text-[hsl(197,80%,28%)] font-bold text-xs sm:text-sm">৳{cabin.pricePerNight.toLocaleString()}</span>
                  <span className="text-slate-400 text-[10px] sm:text-xs">/রাত</span>
                </div>
              </div>

              {/* Body */}
              <div className="p-4 sm:p-5 flex flex-col flex-1">
                <h3 className="text-base sm:text-xl font-bold text-slate-800 mb-0.5">{cabin.name}</h3>
                <p className="text-slate-400 text-xs sm:text-sm mb-3">{cabin.nameEn} · {cabin.size}</p>
                ...
                <div className="grid grid-cols-2 gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                  ...
                  <div className="flex items-center gap-1.5 text-xs sm:text-sm">
                    {cabin.hasAC ? (
                      <>
                        <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-500 flex-shrink-0" />
                        <span className="text-slate-600">এসি</span>
                      </>
                    ) : (
                      <>
                        <Wind className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 flex-shrink-0" />
                        <span className="text-slate-500">নন-এসি</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                >
                  {cabin.available ? 'এই কেবিন বুক করুন' : 'Booked Out'}
                </button>
              </div>
            </div>
          ))}
        ...
        <div className="mt-8 sm:mt-12 bg-gradient-to-br from-[hsl(197,80%,28%)] to-[hsl(173,58%,38%)] rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 text-center text-white">
          <h3 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3">পুরো বোট বুক করুন</h3>
          <p className="text-white/80 text-sm sm:text-base md:text-lg mb-5 sm:mb-6 max-w-xl mx-auto">
            সর্বোচ্চ ২৪ জনের গ্রুপের জন্য পুরো হাউসবোটটি এক্সক্লুসিভলি বুক করুন এবং বিশেষ ছাড় পান।
          </p>
          <button
            onClick={onBookNow}
